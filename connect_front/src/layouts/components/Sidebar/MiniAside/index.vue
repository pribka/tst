<template>
    <div
        ...
        class="a_m"
        :class="[isHovered ? 'open_menu' : 'hide_menu', isMenuManageMode && isHovered ? 'manage_mode' : '', isTablet ? 'is_tablet' : '']">
        ...
        <div class="menu_list" :class="[!isTop && 'top_a', !isBottom && 'bottom_a', dragging && 'dragging']">
            <vueScroll :ops="ops" @handle-scroll="onScroll">
                <draggable
                    v-if="isHovered && isMenuManageMode"
                    v-model="routersList"
                    :forceFallback="false"
                    ghost-class="ghost"
            ...
            <div v-if="user && user.support_chat" class="a_m__b--i a_i">
                <router-link :to="{ name: 'chat', query: { chat_id: user.support_chat } }" class="relative i_w">
                    <div class="i_w__c">
                        ...
                        <div v-if="isHovered" class="name">{{ $t('support_label') }}</div>
                    </div>
                </router-link>
            </div>
    ...
    computed: {
        ...
        routersList: {
            get() {
                ...
                if(this.isHovered && this.isMenuManageMode)
                    return list
        ...
        &::v-deep{
            ...
            .menu_group_item{
                &.open{
                    ...
                    .menu_group_item__content{
                        color: var(--text);
                    }
                }
                .menu_group_item__right{
                    position: absolute;
                    z-index: 3;
                    top: -2px;
                    right: 3px;
