<template>
    <ModuleWrapper
        ...
        <PositionsListMobile
            ...
            :pageName="initPageName" />
        <PositionFormModal :model="initPageModel" contractorSelect />
        <MobileFloatActions
            v-if="isMobile"
            :pageModel="initPageModel"
            :pageName="initPageName"
...
export default {
    components: {
        ...
        PositionFormModal: () => import('../../components/PositionFormModal.vue'),
        PositionsListMobile: () => import('./ListMobile.vue'),
    ...
    data() {
        return {
            endpoint: "/help_desk/contact_person_post/"
        }
    },
    computed: {
        currentRoute() {
        ...
        pageTitle() {
            return this.$route?.meta?.title || this.$t('directories.positions_label')
        },
