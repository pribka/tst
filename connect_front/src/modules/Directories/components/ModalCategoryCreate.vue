        ...
        <a-form-model
            ...
            <a-form-model-item prop="name" class="mb-2">
                <a-input
                    ...
                    v-model="form.name"
                    size="large"
                    :placeholder="$t('directories.category_name')" />
            </a-form-model-item>
            <a-form-model-item
            ...
            <a-button block type="primary" size="large" :loading="loading" class="mt-2" @click="submit()">
                {{ editMode ? $t('directories.save') : $t('directories.create') }}
            </a-button>
        </a-form-model>
    </a-modal>
</template>
                        ...
                        try {
                            ...
                            if(this.editMode) {
                                ...
                                if(data) {
                                    ...
                                    this.$message.success(this.$t('directories.category_updated'))
                                ...
                                if(data) {
                                    if(this.slaSelected)
                                        await this.saveSLA(data)
                                    this.$message.success(this.$t('directories.category_created'))
                                    eventBus.$emit(`update_filter_${this.model}`)
                                    this.afterCreate(data)
