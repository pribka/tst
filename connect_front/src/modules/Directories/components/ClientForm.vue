        ...
        <template #title v-if="!formLoading">
            <a-form-model ref="nameForm" :model="form">
                <a-form-model-item
                    ...
                    prop="name"
                    class="mb-0"
                    :rules="{ required: true, message: $t('directories.required_field'), trigger: 'change' }">
                    <div class="flex items-center justify-between">
                        <a-input
                            v-model="form.name"
                            ref="nameInput"
                            inputType="ghost"
                            :placeholder="$t('directories.contractor_name')"
                            size="large"
                            @pressEnter="onSubmit()" />
        ...
        <a-form-model
            ...
            <a-form-model-item
                ...
                :rules="formInfo?.inn?.rules ?? [
                    { required: true, message: $t('directories.required_field'), trigger: 'change' },
                    { min: 12, message: $t('directories.min_symbols', {count: 12}), trigger: 'blur' },
                    { max: 12, message: $t('directories.max_symbols', {count: 12}), trigger: 'blur' },
                ]">
                <a-input
                    v-model="form.inn"
                    inputType="ghost"
            ...
            <a-form-model-item
                ...
                <DSelect
                    ...
                    size="default"
                    firstSelected
                    :placeholder="formInfo.org_admin.placeholder || $t('directories.support_organization')"
                    :listObject="false"
                    labelKey="name"
            ...
            <transition name="slide-fade">
                <div v-if="form.org_admin && formInfo.admins" class="mb-2">
                    <div class="mb-3">{{ formInfo.admins.title || $t('directories.admin_organization') }}</div>
            ...
            <template v-if="formInfo.contact_persons">
                <div class="mb-3">{{ formInfo.contact_persons.title || $t('directories.contact_person') }}</div>
                ...
                <a-spin v-if="formInfo.contact_persons.fields" :spinning="deleteLoading" size="small">
                    <div
                        ...
                        <a-form-model-item
                            ...
                            :rules="[{ type: 'email', message: $t('directories.email_invalid'), trigger: 'blur' }]">
                            <a-input
        ...
        <template #footer>
            ...
            <div v-if="isMobile" class="flex items-center w-full">
                <a-button
                    ...
                    @click="onSubmit()">
                    {{ edit ? $t('directories.save') : $t('directories.create_contractor') }}
                </a-button>
            </div>
                        ...
                        try {
                            ...
                            if (this.edit) {
                                ...
                                if (data) {
                                    ...
                                    this.visible = false
                                    this.$message.success(this.$t('directories.contractor_updated'))
                                ...
                                if (data) {
                                    ...
                                    this.visible = false
                                    this.$message.success(this.$t('directories.contractor_created'))
                                    eventBus.$emit(`update_filter_${this.model}_${this.pageName}`)
                                    eventBus.$emit(`update_filter_${this.model}`)
                                    if (this.$route.query?.client)
                                        eventBus.$emit('client_detail_reload')
