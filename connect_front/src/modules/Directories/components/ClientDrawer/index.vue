...
export default {
    ...
    computed: {
        ...
        panes() {
            const tabs = [
                {
                    key: 'info',
                    label: this.$t('directories.basic_information'),
                },
                {
                    key: 'contact_persons',
                    label: this.$t('directories.contact_persons')
                },
                {
                    key: 'specialists',
                    label: this.$t('directories.support_specialists')
                },
                {
                    key: 'files',
                    label: this.$t('directories.files')
                },
                {
                    key: 'history',
                    label: this.$t('directories.appeal_history')
                }
            ]
            if(this.actions?.create_notes?.availability || this.actions?.edit_notes?.availability) {
                tabs.push({
                    key: 'notes',
                    label: this.$t('directories.notes')
                })
            }
            return tabs
        }
    ...
    methods: {
        ...
        deleteClient() {
            this.$confirm({
                title: this.$t('directories.confirm_delete_contractor'),
                closable: true,
                maskClosable: true,
                cancelText: this.$t('directories.cancel'),
                ...
                onOk: () => {
                    return new Promise((resolve, reject) => {
                        this.$http.post('/table_actions/update_is_active/', [{ id: this.client.id, is_active: false }])
                            .then(() => {
                                this.$message.success(this.$t('directories.contractor_deleted'))
        ...
        async getClient() {
            try {
                ...
                if(error.status === 404) {
                    this.$message.error(this.$t('directories.contractor_not_found'))
                } else {
                    if(error && error.detail) {
                        if(error.detail === 'Не найдено.' || error.detail === 'Страница не найдена.' || error.detail === 'У вас недостаточно прав для выполнения данного действия.') {
                            this.$message.warning(this.$t('directories.contractor_not_found'))
                        } else {
