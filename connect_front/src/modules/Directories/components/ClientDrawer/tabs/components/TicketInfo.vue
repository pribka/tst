<template>
    <DrawerAside class="h-full ticket_info relative">
        <ListView v-if="ticket" inline labelDark>
            ...
            <ListViewItem v-if="ticket.start_date && !ticket.end_date" :title="$t('directories.start_date')">
                {{ $moment(ticket.start_date).format('DD.MM.YYYY') }}
            </ListViewItem>
            <ListViewItem v-if="ticket.end_date && ticket.end_date" :title="$t('directories.deadlines')">
                {{ $moment(ticket.start_date).format('DD.MM.YYYY') }} - {{ $moment(ticket.end_date).format('DD.MM.YYYY') }}
            </ListViewItem>
            <ListViewItem v-if="ticket.status" :title="$t('directories.status')">
                <a-tag :color="ticket.status.color" block size="large">
                    {{ ticket.status.name }}
                </a-tag>
            </ListViewItem>
            <ListViewItem v-if="checkField({ key: 'category' })" :title="$t('directories.category')">
                {{ ticket.category.name }}   
            </ListViewItem>
            <ListViewItem :title="$t('directories.priority')">
                <ProirityCard :priority="curPriority" />
            </ListViewItem>
            <ListViewItem v-if="checkField({ key: 'dead_line' })" :title="$t('directories.deadline')">
                {{ $moment(ticket.dead_line).format('DD.MM.YYYY') }}
            </ListViewItem>
            <ListViewItem :title="$t('directories.responsible')">
                <Profiler 
                    :avatarSize="22"
                    nameClass="text-sm"
                    :user="ticket.specialist" />
            </ListViewItem>
            <ListViewItem v-if="checkField({ key: 'visors', type: 'array' })" :title="$t('directories.observers')">
                <Profiler 
            ...
            <ListViewItem v-if="ticket.channel" :title="$t('directories.communication_channel')">
        ...
        <div v-else class="empty_ticket">
            <div>
                ...
                <div class="empty_ticket__message">{{ $t('directories.select_appeal_to_view') }}</div>
            </div>
        </div>
    </DrawerAside>
</template>
