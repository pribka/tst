<template>
    <div :style="!useInject && 'max-width: 300px;'" class="w-full flex items-center">
        ...
        <a-select 
            ...
            :loading="listLoading"
            :disabled="disabled"
            :placeholder="$t('directories.select_sla')"
            @change="changeSLA" 
    ...
    methods: {
        ...
        changeSLA(value) {
            ...
            if(this.useInject) {
                ...
                this.$confirm({
                    title: this.$t('directories.confirm_changes'),
                    content: this.$t('directories.set_sla_rules', { name: find.name, name2: this.selectItem.name }),
