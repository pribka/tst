            ...
            <template>
                <ul class="bordered-items">
                    <li 
                        ...
                        :key="item.id"
                        :style="isOptionDisabled(item.id) && 'opacity: 0.5;'"
                        class="item py-3" 
                        :class="isOptionDisabled(item.id) || 'cursor-pointer'"
                        @click="isOptionDisabled(item.id) ||selectUser(item)">
                        <div class="flex items-center justify-between">
                            {{ item.name }}
                            <div v-if="showRadio && multiple && !isOptionDisabled(item.id)" >
...
export default {
    ...
    computed: {
        selectButtonText() {
            return this.value ? this.$t('team.change') : this.$t('team.select')
        },
        size() {
    ...
    methods: {
        ...
        isOptionDisabled(id) {
            return this.usedPermissionTypes.includes(id)
        },
        getPopupContainer() {
        ...
        async addUsersToOrganization() {
            ...
            if(!user.created) {
                const type = this.isDepartment ? this.$t('team.department') : this.$t('team.organization')
                const message = this.$t('team.user_already_exists', { name: user.full_name, type })
