    ...
    <WidgetWrapper 
        ...
        <div ref="scroller" class="scroller_block">
            <div
                ...
                <p>{{ $t('dashboard.groupTaskEmptyMessage') }}</p>
                <a-button
                    type="ui"
                    size="small"
...
export default {
    ...
    created() {
        if(this.widget.random_settings?.related_object)
            this.selectedGroup = this.widget.random_settings.related_object
        ...
        async saveGroupConfig() {
            try {
                const randomSettings = {
                    related_object: this.selectedGroup || null,
                    related_model: this.selectedGroup ? 'workgroups.WorkgroupModel' : null
                }
        ...
        openGroupSetting() {
            this.$nextTick(() => {
                if(this.$refs.groupSelect)
                    this.$refs.groupSelect.openSelect()
            })
        },
        addTask() {
            if(!this.selectedGroup)
                return
            const pageName = `tasks.workgroup_${this.selectedGroup.id}`
            const data = {
                workgroup: {
                    id: this.selectedGroup.id,
                    name: this.selectedGroup.name,
                    workgroup_logo: this.selectedGroup.workgroup_logo || null,
                    date_start_plan: this.selectedGroup.date_start_plan,
                    dead_line: this.selectedGroup.dead_line
                }
            }
                ...
                try {
                    ...
                    const { data } = await this.$http.get('/tasks/task/list/', {
                        params: {
                            page: this.page,
                            page_size: 15,
                            page_name: `tasks.workgroup_${this.selectedGroup.id}`,
                            task_type: 'task,stage,milestone',
                            filters: {
                                workgroup: this.selectedGroup.id,
                                parent: null
                            }
                        }
                    })
