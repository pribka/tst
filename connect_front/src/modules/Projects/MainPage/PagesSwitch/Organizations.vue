<template>
    <div class="participants-wrapper">
        <div class="bottoms">
            <div class="add">
                <a-button icon="plus" size="large" type="primary" @click="openAddDrawer(false)">Добавить организацию</a-button>
            </div>
        </div>
...
export default{
    name: 'Participants',
    components: {
        AddOrgDrawer: () => import('../modules/module_components/AddOrgDrawer'),
        MemberOrgCard: () => import('../modules/module_components/MemberOrgCard.vue')
    },
    props: {
        isFounder: {
            type: Boolean,
    ...
    mounted() {
        ...
        eventBus.$on('reload_member_organization_list', () => {
            this.getData()
        })
    },
    beforeMount() {
        eventBus.$off('reload_member_organization_list')
    },
    methods: {
