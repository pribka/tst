        ...
        <template v-if="!roleLoader">
            <template v-if="!loadingNews">
                <a-empty
                    ...
                    "
                    class="mt-10"
                    :description="$t('project.news_not_found')" />
            </template>

            <!--<a-result 
                v-if="!isStudent && requestData.public_or_private"
                :title="$t('project.no_partisipants_group')">
                <template #icon>
                    <a-icon 
                        type="lock" 
                        theme="twoTone" />
            ...
            <DrawerTemplate 
                ...
                <template #title>
                    <div class="drawer_title">
                        {{ isUpdateMode ? $t('project.edit_post') : $t('project.create_post') }}
                    </div>
                </template>
                <div class="news_form">
                    <a-form-model 
                        ...
                        :model="dataNewNews" 
                        :rules="rulesPost">
                        <a-form-model-item prop="title"  :label="$t('project.title_news')">
                            <a-input size="large" v-model="dataNewNews.title" />
                        </a-form-model-item>
                        <a-form-model-item prop="content" class="-mt-4" :label="$t('project.description')">
                ...
                <template #footer>
                    <a-button 
                        ...
                        {{ $t(isUpdateMode ? 'project.save_post' : 'project.create_post') }}
                    </a-button>
                </template>
            </DrawerTemplate>
        </template>
...
export default {
    ...
    data(){
        return {
            rulesPost: {
                ...
                content: [{ required: true, message: this.$t('project.field_require'), trigger: 'blur' },],
            },
    ...
    methods:{
        ...mapActions({
            ...
            createNews: "projects/postNews",
            updateNews: "projects/updateNews",
            deleteNewsAction: "projects/deleteNews",
            getNews: "projects/getAllNews"
        }),
        getPopupContainer() {
        ...
        async postNews() {
            try{
                ...
                this.$message.success(this.$t('project.news_created'))      
            ...
            this.$confirm({
                ...
                onOk: () => {
                    return new Promise((resolve, reject) => {
                        this.$store.dispatch('projects/deleteNews', news.id)
                            .then(() => {
                                if(this.visible)
                                    this.visible = false
