<template>
    <div :class="[!buttonMode && !hide && 'user_draw_input ant-input flex items-center relative', inputClass, size]">
            ...
            <template v-if="checkMultiple">
                        ...
                        <Profiler :user="value[0]" class="mr-2">
                            <a-tag color="blue" class="tag_block" @click="open()">
                                <div class="flex items-center">
                                    <div class="mr-1">
                                        <a-avatar :size="15" v-if="value[0].avatar" :src="value[0].avatar.path" />
                                        <a-avatar :size="15" v-else icon="user" />
                                    </div>
                                    ...
                                    <template v-for="(item, index) in value">
                                        <template v-if="index !== 0">
                                            <div :key="`o_${index}`" class="flex items-center user_popup_item">
                                                <div class="mr-1">
                                                    <a-avatar :size="15" v-if="item.avatar" :src="item.avatar.path" />
                                                    <a-avatar :size="15" v-else icon="user" />
                                                </div>
                                                {{checkNameList(item)}}
                                            </div>
                ...
                <Profiler v-else :user="value" class="mr-2">
                    <a-tag color="blue" class="tag_block" @click="open()">
                        <div class="flex items-center">
                            <div class="mr-1">
                                <a-avatar :size="15" v-if="value.avatar" :src="value.avatar.path" />
                                <a-avatar :size="15" v-else icon="user" />
                            </div>
                            {{checkName}}
                        </div>
        ...
        <a-drawer
            :title="driwerTitle"
            class="user_select_driwer"
            :class="multiple && 'multiple_select'"
            :width="isMobile ? '100%' : 380"
            :destroyOnClose="true"
            @close="visible = false"
            :zIndex="modal ? 9999999 : 1200"
            :visible="visible">
            ...
            <div class="drawer_body">
                <div class="drawer_scroll">
                    <ul class="bordered-items">
                        <li class="cursor-pointer item px-3 py-3" @click="selectUser(user)" v-for="user in listFilter" :key="user.id">
                            <div class="flex items-center justify-between">
                                <div class="flex items-center">
                                    <div class="mr-2">
                                        <a-avatar v-if="user.avatar" :src="user.avatar.path" />
                    ...
                    <infinite-loading @infinite="scrollHandler" v-bind:distance="10">
...
export default {
    ...
    mounted(){
        eventBus.$on('open_user_comment_drawer', (id)=>{  
            if(id === this.id)
                this.open()
        })
    },
    beforeDestroy(){
        eventBus.$off('open_user_comment_drawer')
    }
}
</script>
