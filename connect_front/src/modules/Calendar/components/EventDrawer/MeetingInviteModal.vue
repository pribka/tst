<template>
    <a-modal
        :visible="visible"
        :title="$t('calendar.meeting_invite_title')"
        :width="560"
        @cancel="handleCancel()">
        <div class="meeting_invite_modal">
            <a-alert
                ...
                type="warning"
                :message="$t('calendar.meeting_invite_no_selection_alert')"
                show-icon />
            <div class="meeting_invite_modal__header">
                <div>
                    {{ $t('calendar.meeting_invite_participants_event') }}
                </div>
                <a-button
                    size="small"
                    type="ui"
                    @click="toggleSelectAll()">
                    {{ allSelected ? $t('calendar.meeting_invite_unselect_all') : $t('calendar.meeting_invite_select_all') }}
                </a-button>
            </div>
        ...
        <template #footer>
            <div class="meeting_invite_modal__footer flex items-center w-full flex-wrap gap-x-2 gap-y-2">
                <a-button
                    ...
                    :loading="loading"
                    @click="invite()">
                    {{ $t('calendar.meeting_invite_submit_and_start', { count: selectedCount }) }}
                </a-button>
            </div>
        </template>
    </a-modal>
