        ...
        <DrawerTemplate
            ...
            <template>
                <a-input-search
                    ...
                    v-model="search"
                    @input="onSearch"
                    :placeholder="$t('task.working_group_name')" />
                <OldSelected 
                    class="mb-4"
                    ref="groupOldSelector"
...
export default {
    ...
    data() {
        return {
            driwerTitle: this.title || this.$t('task.select_working_group'),
            search: '',
            visible: false,
            searchLoading: false,
            scrollStatus: true,
    ...
    methods: {
        getPopupContainer() {
            return document.querySelector('.gp_scroll')
        },
        onSearch() {
            clearTimeout(timer)
            timer = setTimeout(() => {
        ...
        selectWork(work) {
            ...
            this.$refs.groupOldSelector.saveSelect(work)
            this.visible = false
        },
        checkSelected(work) {
            if(this.value) {
        ...
        async getWorkList($state = null) {
            if(!this.loading && this.scrollStatus && this.visible) {
                try {
                    ...
                    const is_helpdesk = this.isHelpdesk
                    let params = {
                        page_size: 15,
                        page: this.page,
                        is_project: 0,
