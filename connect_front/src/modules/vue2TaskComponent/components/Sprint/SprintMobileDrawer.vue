    ...
    <a-drawer
        ...
        <div class="tab_menu">
            <a-menu
                ...
                <a-menu-item v-for="menuItem in menuRoutes" :key="menuItem.id" >
        ...
        <!-- <component
            v-if="sprint.id"
            :is="sprintTasksWidget"
            :getTaskCount="getTaskCount"
            :sprint-id="sprint.id"
            :moveTaskToBacklog="moveTaskToBacklog"
            :moveTaskToSprint="moveTaskToSprint"/> -->
        <div 
        ...
        <div 
            ...
            <div class="task_list">
                ...
                <infinite-loading
                    ...
                    :ref="`infinite_${data.name}`"
                    @infinite="upScrollHandler(data.name, $event)">
                    <div 
                        slot="spinner" 
                        class="pt-1">
                        <a-spin v-if="data.list && data.list.length" />
                    </div>
...
export default {
    ...
    data(){
        return {
            menuRoutes,
            activeMenu: 'sprint',
            activeMenuList: ['sprint'],
            defaultActiveMenu: ['sprint'],
    ...
    computed: {
        ...
        sprintTasksWidget() {
            if(this.isMobile) {
                if(this.activeMenu === 'sprint') 
                    return () => import(`./SprintTasksIncluded.vue`)
                return () => import(`./SprintTasksBacklog.vue`)
            }
            return false
        },
    ...
    methods: {
        ...
        moveTaskToBacklog(task) {
            let index = this.allData.findIndex(el=> el.name === "list")
            this.allData[index].list.unshift(task)

            index = this.allData.findIndex(el=> el.name === "sprint")
            this.allData[index].list.splice(this.allData[index].list.findIndex(task => task.id === task.id), 1)
        },
        moveTaskToSprint(task) {
            let index = this.allData.findIndex(el=> el.name === "sprint")
            this.allData[index].list.unshift(task)

            index = this.allData.findIndex(el=> el.name === "list")
            this.allData[index].list.splice(this.allData[index].list.findIndex(task => task.id === task.id), 1)
