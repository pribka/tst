                ...
                <a-spin :spinning="dataLoading" size="small">
                    <div
                        ...
                        <a-form-model-item
                            ...
                            :rules="[
                                { type: 'email', message: $t('helpdesk.email_invalid'), trigger: 'blur' }
                            ]">
                            <a-input
                        ...
                        <a-form-model-item v-if="formInfo.contact_persons.fields.comment" :prop="'contact_persons.' + index + '.comment'">
                            <a-textarea v-model="contact_person.comment" :placeholder="formInfo.contact_persons.fields.comment.placeholder || $t('helpdesk.enter_comment')"  @pressEnter="formSubmit()">
                            </a-textarea>
                        </a-form-model-item>
                    </div>
                </a-spin>
...
export default {
    ...
    computed: {
        ...
        modalTitle() {
            return this.edit ? this.$t('helpdesk.edit_contact_person') : this.$t('helpdesk.add_contact_person')
        },
        isMobile() {
            ...
            this.$http.put(`help_desk/customer_cards/${this.client.id}/contact_persons/update/`, payload)
                .then(async ({ data }) => {
                    if (data) {
                        ...
                        this.$message.success(this.$t('helpdesk.contact_person_updated'))
                        this.visible = false
                    }
                })
                .catch(error => {
