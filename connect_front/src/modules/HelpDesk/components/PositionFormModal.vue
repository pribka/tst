<template>
    <a-modal
        ...
        <a-form-model
            ...
            <a-form-model-item prop="name" class="mb-2">
                <a-input 
                    ...
                    ref="nameInput"
                    size="large" 
                    :placeholder="$t('helpdesk.position_print')" />
            </a-form-model-item>
            <a-form-model-item
        ...
        <a-button block type="primary" size="large" :loading="loading" class="mt-2" @click="submit()">
            {{ edit ? $t('helpdesk.save') : $t('helpdesk.create') }}
        </a-button>
    </a-modal>
</template>
                        ...
                        try {
                            ...
                            if(this.edit) {
                                ...
                                if(data) {
                                    ...
                                    this.$message.success(this.$t('helpdesk.position_updated'))
                                ...
                                if(data) {
                                    if(this.slaSelected)
                                        await this.saveSLA(data)
                                    this.$message.success(this.$t('helpdesk.position_created'))
                                    eventBus.$emit(`update_filter_${this.model}`)
                                    this.$emit('change', data)
                                    this.visible = false
                                }
