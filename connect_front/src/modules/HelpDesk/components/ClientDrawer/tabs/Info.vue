    ...
    <a-row :gutter="{xs: 15, md: 20, lg: 30, xxl: 30}" class="h-full">
        <a-col
            ...
            <div class="text-lg font-semibold text-gray-800 mb-4">
                {{ $t('helpdesk.a_brief_summary_of_the_requests') }}
            </div>
            <div class="sprint_stat grid gap-3 md:gap-8 grid-cols-1 md:grid-cols-3 mb-5">
                <div class="sprint_stat__card bg-blue-100 border border-blue-300 grid gap-[10px]">
                    <div class="text-blue-600 text-xs sm:text-sm font-medium sm:mb-1">{{ $t('helpdesk.a_total_number_of_requests') }}</div>
                ...
                <div class="sprint_stat__card bg-yellow-100 border border-yellow-300 grid gap-[10px]">
                    <div class="text-yellow-600 text-xs sm:text-sm font-medium sm:mb-1">{{ $t('helpdesk.a_brief_summary_of_the_requests_active') }}</div>
                ...
                <div class="sprint_stat__card bg-green-100 border border-green-300 grid gap-[10px]">
                    <div class="text-green-600 text-xs sm:text-sm font-medium sm:mb-1">{{ $t('helpdesk.a_brief_summary_of_the_requests_completed') }}</div>
        ...
        <a-col
            ...
            class="h-full">
            <DrawerAside>
                <p v-if="edit" class="mb-2">{{ $t('helpdesk.tags') }}</p>
                <Tags
                    v-if="edit"
                    showBorder
                    :model="model"
                ...
                <a-form-model
                    ...
                    <ListView inline labelDark>
                        ...
                        <ListViewItem :title="$t('helpdesk.status')">
                        ...
                        <ListViewItem v-if="formInfo.inn && checkField({ key: 'inn' })" :title="formInfo.inn.title || $t('helpdesk.organization_bin')">
                            <a-form-model-item v-if="edit" ref="inn" prop="inn">
                                <div class="input_row">
                                    <a-input
                                        ...
                                        :maxLength="12"
                                        size="small"
                                        :placeholder="formInfo.inn.placeholder || $t('helpdesk.enter_organization_bin')"
                        ...
                        <ListViewItem v-if="formInfo.full_name && checkField({ key: 'full_name' })" :title="formInfo.full_name.title || $t('helpdesk.full_name')">
                            <a-form-model-item v-if="edit" ref="full_name" prop="full_name">
                                <div class="input_row">
                                    <a-input
                                        ...
                                        inputType="ghost"
                                        size="small"
                                        :placeholder="formInfo.full_name.placeholder || $t('helpdesk.full_name')"
                        ...
                        <ListViewItem v-if="formInfo.legal_address && checkField({ key: 'legal_address' })" :title="formInfo.legal_address.title || $t('helpdesk.legal_address')">
                        ...
                        <ListViewItem v-if="formInfo.org_admin && checkField({ key: 'org_admin' })" :title="formInfo.org_admin.title || $t('helpdesk.support_organization')">
                            <span>{{ client.org_admin.name }}</span>
                        </ListViewItem>
                    </ListView>

                    <div v-if="formInfo.admins" class="collapse_block">
                        <p v-if="edit || client?.admins?.length" class="mb-2">{{ formInfo.admins.title || $t('helpdesk.admin_organization') }}</p>
                        ...
                        <template v-else>
                            <div v-if="formInfo.admins.fields" class="mb-2">
                                <div
                                    ...
                                    class="contact_block">
                                    <ListView size="small">
                                        <ListViewItem v-if="admins.bin && formInfo.admins.fields.bin" :title="formInfo.admins.fields.bin.title || $t('helpdesk.bin')">
                                            {{ admins.bin }}
                                        </ListViewItem>
                                        <ListViewItem v-if="admins.name && formInfo.admins.fields.name" :title="formInfo.admins.fields.name.title || $t('helpdesk.enter_name')">
                                            {{ admins.name }}
                                        </ListViewItem>
                                    </ListView>
                                </div>
    ...
    data() {
        return {
            ...
            rules: {
                ...
                inn: [
                    { required: true, message: this.$t('helpdesk.required_field'), trigger: 'change' },
                    { min: 12, message: this.$t('helpdesk.min_symbols', {count: 12}), trigger: 'blur' },
                    { max: 12, message: this.$t('helpdesk.max_symbols', {count: 12}), trigger: 'blur' },
                ],
            },
    ...
    methods: {
        async changeStatus(item) {
            try {
                ...
                if(data) {
                    this.$message.success(this.$t('helpdesk.status_changed'))
                    this.listReload()
                }
            } catch(error) {
                errorHandler({error})
                    ...
                    if(org) {
                        try {
                            ...
                            if(data) {
                                ...
                                this.$message.success(this.$t('helpdesk.organization_saved'))
                            }
                        } catch(error) {
                            errorHandler({error})
                        } finally {
            ...
            if(org) {
                ...
                if(org.edit) {
                    ...
                    orgDeleteTimer = setTimeout(() => {
                        this.dataChange({field: 'admins', useTimer: true, valueKey: 'id', multiple: true})
                        this.$message.success(this.$t('helpdesk.organization_removed'))
                    }, 300)
                }
            }
        },
        ...
        async patchField(value, field) {
            try {
                if(field === 'name' && !value) {
                    this.$message.warning(this.$t('helpdesk.name_required'))
                    return false
                }
