<template>
    <a-modal
        :title="edit ? $t('helpdesk.edit_specialist') : $t('helpdesk.add_specialist')"
        destroyOnClose
        :width="600"
        :visible="visible"
        ...
        <a-form-model
            ...
            <a-form-model-item prop="comment">
                <a-textarea 
                    ...
                    :placeholder="$t('helpdesk.enter_comment')" />
            </a-form-model-item>
            ...
            <a-form-model-item prop="user">
                <UserMiniSelect 
                    ...
                    :placeholder="$t('helpdesk.select_support_employee')" />
            </a-form-model-item>
            ...
            <a-form-model-item prop="start_date">
                <a-date-picker 
                    ...
                    allowClear
                    class="w-full"
                    :placeholder="$t('helpdesk.select_start_period')"
                    format="DD.MM.YYYY"
            ...
            <a-form-model-item prop="end_date">
                <a-date-picker 
                    ...
                    allowClear
                    class="w-full"
                    :placeholder="$t('helpdesk.select_end_period')"
                    format="DD.MM.YYYY"
                    :getCalendarContainer="getPopupContainer"
                    iconPosition="left"
            ...
            <a-form-model-item prop="duration_plan">
                <div class="number_input flex items-center">
                    ...
                    <a-input-number 
                        ...
                        :min="1" 
                        :placeholder="$t('helpdesk.enter_hours_plan')"
                        class="w-full" />
                </div>
            </a-form-model-item>
            ...
            <a-form-model-item prop="is_reserve">
                <a-checkbox v-model="form.is_reserve">
                    {{ $t('helpdesk.is_reserve') }}
                </a-checkbox>
            </a-form-model-item>

            <div class="mb-3">{{ $t('helpdesk.vacation_dates') }}</div>
        ...
        <template #footer>
            <div class="flex items-center gap-1 w-full">
                <a-button type="primary" size="large" :loading="loading" @click="onSubmit()">{{ edit ? $t('helpdesk.save') : $t('helpdesk.add') }}</a-button>
    ...
    data() {
        return {
            ...
            locale: {
                lang: {
                    ...locale.lang,
                    rangePlaceholder: [this.$t('helpdesk.vacation_from'), this.$t('helpdesk.vacation_to')]
                },
            ...
            this.$refs.ruleForm.validate(async valid => {
                ...
                try {
                    ...
                    if(this.edit) {
                        ...
                        if(data) {
                            this.$message.success(this.$t('helpdesk.specialist_updated'))
                        ...
                        if(data) {
                            this.$message.success(this.$t('helpdesk.specialist_added'))
                            eventBus.$emit(`update_filter_${this.model}_${this.pageName}`)
                            eventBus.$emit(`update_filter_${this.model}_${this.pageName}_aggregate`)
                            eventBus.$emit(`update_filter_${this.mainModel}_${this.mainPageName}`)
                            this.visible = false
