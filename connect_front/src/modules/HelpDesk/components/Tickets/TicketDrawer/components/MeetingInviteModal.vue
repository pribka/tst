<template>
    <a-modal
        :visible="visible"
        :title="$t('helpdesk.meeting_invite_title')"
        :width="560"
        @cancel="handleCancel()">
        <div class="meeting_invite_modal">
            <div class="mt-2">
                <a-alert
                    ...
                    type="warning"
                    :message="$t('helpdesk.meeting_invite_no_selection_alert')"
                    show-icon />
            </div>
            <div class="meeting_invite_modal__header">
                <div>
                    {{ $t('helpdesk.meeting_invite_participants') }}
                </div>
                <a-button
                    size="small"
                    type="ui"
                    @click="toggleSelectAll()">
                    {{ allSelected ? $t('helpdesk.meeting_invite_unselect_all') : $t('helpdesk.meeting_invite_select_all') }}
                </a-button>
            </div>
        ...
        <template #footer>
            <div class="meeting_invite_modal__footer flex items-center w-full flex-wrap gap-x-2 gap-y-2">
                <a-button
                    ...
                    :loading="loading"
                    @click="invite()">
                    {{ $t('helpdesk.meeting_invite_submit_and_start', { count: selectedCount }) }}
                </a-button>
            </div>
        </template>
    </a-modal>
