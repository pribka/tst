...
export default {
    ...
    computed: {
        ...
        CKEditorMode() {
            return !this.isTelegramChannel && !this.isInternalChannel
        },
        isMobile() {
            return this.$store.state.isMobile
        },
        isTelegramChannel() {
            return this.ticket.channel?.code === 'telegram'
        },
        isInternalChannel() {
            return this.ticket.channel?.code === 'internal_chat'
        },
        isClientView() {
        ...
        async sendMessage() {
            ...
            try {
                ...
                if (this.isClientView){
                    ...
                    var queryData = {
                        contact_person: this.ticket.contact_person.id,
                        text: this.message.text,
                        attachments: [],
                        ticket: this.ticket.id
                    }
                    if(this.ticket.channel?.code)
                        queryData.channel = this.ticket.channel.code
                    if(this.replaceMessage)
                        queryData.reply = this.replaceMessage.id
                    if (this.files.length) {
                        queryData.attachments = this.files.map(file => file.id)
                    }
                }

                var urlSendMessage = '/help_desk/contact_persons/messages/'
                if (this.isClientView){
                    urlSendMessage = '/help_desk/contact_persons/messages/client/create/'

                }
                const { data } = await this.$http.post(urlSendMessage, queryData)
                this.message = {
                    text: "",
                    attachments: []
                }
