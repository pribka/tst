        ...
        <template v-if="multiple">
            <div class="multiple">
                ...
                <div class="list">
                    <UserItem 
                        v-for="user in list"
                        checkedClass="group-checked-class"
                        :key="user.id"
                        :multiple="multiple"
...
export default {
    ...
    methods: {
        selectAll() {
            if (!this.selectedGroupID || !this.list.length) return
            if (this.next) {
                this.pageSize = 'all'
        ...
        async getList($state = null) {
            ...
            if (!this.loading) {
                try {
                    ...
                    const { data } = await this.$http.get(`work_groups/workgroups/${this.selectedGroupID}/get_workgroups_members_short/`, { params })
                    this.next = data.next
