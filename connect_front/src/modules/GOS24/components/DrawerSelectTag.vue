            ...
            <template v-if="checkMultiple">
                <template v-if="multiple">
                    <template v-if="value && value.length">
                        ...
                        <a-button @click="clearList()" type="link" icon="close-circle" class="px-0 text-current remove_users" />
                    </template>
                </template>
...
export default {
    ...
    computed: {
        ...
        listFilter() {
            return this.excludeUsers.length
                ? this.userList.filter(u => !this.excludeUsers.includes(u.id))
                : this.userList
        },
        windowWidth() {
    ...
    methods: {
        ...mapActions({ addEmployees: 'organization/addEmployees' }),
        getPopupContainer() {
        ...
        open() { if(!this.disabled) this.visible = true },
        onSearch() {
            clearTimeout(timer)
        ...
        async scrollHandler($state) {
            ...
            if (this.search && this.search.trim().length) {
                params.search = this.search.trim() // DRF SearchFilter (TagListView.search_fields=['name'])
            }
            if (this.filters && Object.keys(this.filters).length) {
                params.filters = this.filters
            }

            const url = `/content_item_gos24/tag/`
        ...
        // единая логика через scrollHandler
        async getUserSearchList() { this.resetInfinite() },
        async getUserList() { this.resetInfinite() },
        ...
        addUsers() {
            if (this.accessGroup?.id) this.addUsersToAccessGroup()
            else this.addUsersToOrganization()
        },
        addUsersToAccessGroup() {
            const url = `contractor_permissions/access_groups/${this.accessGroup.id}/members/add/`
            const payload = { contractor: this.fromOrganization, members: [ ...this.value.map(user => user.id) ] }
        ...
        closeDrawer() { this.visible = false }
    },
    mounted(){
        eventBus.$on('open_user_task_drawer', (id)=>{ if(id === this.id) this.open() })
    },
    beforeDestroy(){
