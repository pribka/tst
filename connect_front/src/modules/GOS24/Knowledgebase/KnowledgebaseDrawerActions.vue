...
export default {
    name: 'KnowledgebaseDrawerActions',
    components: { 
        DrawerTemplate: () => import('@/components/DrawerTemplate.vue'), 
        KnowledgebaseForm: () => import('../Knowledgebase/KnowledgebaseForm.vue')
    },
    ...
    created () {
        eventBus.$on('create_knowledgebase_gos24', this.open)
        eventBus.$on('edit_knowledgebase_gos24', this.openEdit)
    },
    beforeDestroy () {
        eventBus.$off('create_knowledgebase_gos24', this.open)
        eventBus.$off('edit_knowledgebase_gos24', this.openEdit)
    },
    methods: {
        closeDrawer () {
            this.visible = false
        ...
        open () {
            ...
            this.mode = 'create'
            this.$nextTick(() => {
                this.$refs.knowledgebaseForm?.reset?.()
            })
        },
        async openEdit ({ id }) {
            this.visible = true
            ...
            try {
                const { data } = await axios.get(`/content_item_gos24/knowledgebase/${id}/`)
                this.$nextTick(() => {
                    this.$refs.knowledgebaseForm?.setData?.(data)
                })
            } catch (e) {
        ...
        async save () {
            ...
            if (!this.$refs.knowledgebaseForm?.submit) {
                this.$message.warning('Форма не инициализирована')
                return
            }

            const { valid, payload } = await this.$refs.knowledgebaseForm.submit()
            if (!valid) return
            ...
            try {
                ...
                if (this.mode === 'edit' && this.editingId) {
                    res = await axios.patch(`/content_item_gos24/knowledgebase/${this.editingId}/`, payload)
                    this.$message.success('Изменения сохранены')
                    this.$emit('updated', res.data)
                } else {
                    res = await axios.post('/content_item_gos24/knowledgebase/', payload)
                    this.$message.success('Сохранено')
                    this.$emit('created', res.data)
                }
                this.visible = false
                this.$nextTick(() => eventBus.$emit('knowledgebase:refresh'))
            } catch (e) {
