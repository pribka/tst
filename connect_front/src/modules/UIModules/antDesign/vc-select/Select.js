...
import { Item as MenuItem, ItemGroup as MenuItemGroup } from 'ant-design-vue/es/vc-menu';
import warning from 'warning';
import Vue from 'vue';
import Option from './Option';
import OptGroup from './OptGroup';
import { hasProp, getSlotOptions, getPropsData, getValueByProp as getValue, getComponentFromProp, getEvents, getClass, getStyle, getAttrs, getOptionProps, getSlots, getListeners } from 'ant-design-vue/es/_util/props-util';
...
import { isIE, isEdge } from 'ant-design-vue/es/_util/env';
...
var Select = {
    ...
    props: _extends({}, SelectPropTypes, {
        scrollClose: PropTypes.bool.def(false),
    ...
    methods: {
        getScrollableParent: function getScrollableParent(node) {
            var n = node
            while (n && n !== document.body) {
                var style = window.getComputedStyle(n)
                var overflowY = style.overflowY
                var canScroll = overflowY === 'auto' || overflowY === 'scroll'
                if (canScroll && n.scrollHeight > n.clientHeight) return n
                n = n.parentNode
            }
            return window
        },
        bindScrollClose: function bindScrollClose() {
            if (!this.$props.scrollClose) return
            if (!this._scrollCloseBound) {
                var self = this
                this._scrollCloseHandler = function () {
                    if (!self.$props.disabled && self.getRealOpenState()) {
                        self.setOpenState(false, { needFocus: false })
                        self.blur()
                    }
                }
            ...
            var sp = this.getScrollableParent(root)
            this._scrollParent = sp
            var target = sp === window ? window : sp
            target.addEventListener('scroll', this._scrollCloseHandler, { passive: true })
        },
        unbindScrollClose: function unbindScrollClose() {
            if (!this._scrollCloseBound) return
            var sp = this._scrollParent || window
            var target = sp === window ? window : sp
            target.removeEventListener('scroll', this._scrollCloseHandler)
            this._scrollParent = null
            this._scrollCloseBound = false
        },
        ...
        onDropdownVisibleChange: function onDropdownVisibleChange(open) {
            ...
            if (open) this.bindScrollClose()
            else this.unbindScrollClose()
        },
        ...
        getPlaceholderElement: function getPlaceholderElement() {
            ...
            if (state._inputValue && state._inputValue.length > 0) {
                hidden = true;
            } else if (Array.isArray(value) && value.length === 1) {
                hidden = (value[0] !== null) && (value[0] !== undefined) && value[0] !== '';
        ...
        renderTopControlNode: function renderTopControlNode() {
            ...
            var className = prefixCls + '-selection__rendered truncate';
            ...
            if (isSingleMode(props)) {
                ...
                if (value.length) {
                    ...
                    selectedValue = h(
                        ...
                        {
                            key: 'value',
                            'class': prefixCls + '-selection-selected-value truncate',
    ...
    render: function render() {
        ...
        var selectionProps = {
            ...
            'class': prefixCls + '-selection truncate ' + prefixCls + '-selection--' + (multiple ? 'multiple' : 'single'),
            // directives: [
            //   {
            //     name: 'ant-ref',
