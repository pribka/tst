...
import { getOptionProps, hasProp, mergeProps, getComponentFromProp, getListeners } from 'ant-design-vue/es/_util/props-util';
...
var RangeCalendar = {
    ...
    methods: {
        ...
        getInputHoverPlaceholder: function getInputHoverPlaceholder(index, placeholder, format) {
            var sSelectedValue = this.sSelectedValue || [];
            var sHoverValue = this.sHoverValue || [];
            var firstSelectedValue = this.firstSelectedValue;
            var hoverDate = null;

            if (sSelectedValue[index] || !sHoverValue.length) {
                if (index === 1 && sSelectedValue[0] && !sSelectedValue[1] && !sHoverValue.length) {
                    return formatDate(sSelectedValue[0], format);
                }
                return placeholder;
            }

            if (sHoverValue.length > 1) {
                hoverDate = sHoverValue[index];
                return hoverDate ? formatDate(hoverDate, format) : placeholder;
            }

            if (index === 0 && !sSelectedValue[0]) {
                hoverDate = sHoverValue[0];
            }

            if (index === 1 && sSelectedValue[0] && !sSelectedValue[1]) {
                if (sHoverValue.length === 1) {
                    hoverDate = sHoverValue[0].isSame(sSelectedValue[0], 'day') ? null : sHoverValue[0];
                } else if (!firstSelectedValue) {
                    hoverDate = sHoverValue[1] || sHoverValue[0];
                } else {
                    hoverDate = sHoverValue[0].isSame(firstSelectedValue, 'day') ? sHoverValue[1] : sHoverValue[0];
                }
            }
    ...
    render: function render() {
        ...
        var newProps = {
            ...
            on: {
                ...
                dayHover: type === 'both' || type === 'start' && sSelectedValue[1] || type === 'end' && sSelectedValue[0] || !!sHoverValue.length ? this.onDayHover : noop
            }
        };
        ...
        var dateFormat = this.getFormat();
        var startInputPlaceholder = this.getInputHoverPlaceholder(0, placeholder1, dateFormat);
        var endInputPlaceholder = this.getInputHoverPlaceholder(1, placeholder2, dateFormat);
            ...
            [props.renderSidebar(), h(
                ...
                [showClear && sSelectedValue[0] && sSelectedValue[1] ? h(
                    ...
                    'div',
                    {
                        ...
                        on: {
                            'mouseleave': this.onDatePanelLeave,
