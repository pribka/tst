...
import { getOptionProps, initDefaultProps, getComponentFromProp, isValidElement, getListeners } from 'ant-design-vue/es/_util/props-util';
import { cloneElement } from 'ant-design-vue/es/_util/vnode';
import VcCollapse, { collapseProps } from 'ant-design-vue/es/vc-collapse';
...
export default {
    ...
    methods: {
        renderExpandIcon: function renderExpandIcon(panelProps, prefixCls) {
            ...
            var icon = expandIcon || h('i', {
                
            });
            return isValidElement(Array.isArray(expandIcon) ? icon[0] : icon) ? cloneElement(icon, {
                'class': prefixCls + `-arrow fi fi-rr-angle-small-right ${panelProps.isActive ? 'arrow_active' : ''}`,
                attrs: { rotate: panelProps.isActive ? 90 : undefined }
            }) : icon;
        }
    },
