...
export default {
    ...
    computed: {
        classes: function classes() {
            ...
            return _ref = {}, _defineProperty(_ref, '' + prefixCls, true), _defineProperty(_ref, prefixCls + '-' + type, type), _defineProperty(_ref, 'truncate', useTruncate), _defineProperty(_ref, prefixCls + '-' + shape, shape), _defineProperty(_ref, prefixCls + '-' + sizeCls, sizeCls), _defineProperty(_ref, prefixCls + '-icon-only', children.length === 0 && iconType), _defineProperty(_ref, prefixCls + '-loading', sLoading), _defineProperty(_ref, prefixCls + '-background-ghost', ghost || type === 'ghost'), _defineProperty(_ref, prefixCls + '-two-chinese-chars', hasTwoCNChar && autoInsertSpace), _defineProperty(_ref, prefixCls + '-block', block), _ref;
        }
    },
    watch: {
    ...
    methods: {
        ...
        insertSpace: function insertSpace(child, needInserted) {
            ...
            if (typeof child.text === 'string') {
                ...
                return h('span', { class: getComponentFromProp(this, 'useTruncate') ? 'truncate t_btn_block' : '' }, [text]);
            }
            return child;
        },
    ...
    render: function render() {
        ...
        var icon = getComponentFromProp(this, 'icon');
        var iconClass = getComponentFromProp(this, 'iconClass') || '';
        var flaticon = getComponentFromProp(this, 'flaticon');
        var iconRight = getComponentFromProp(this, 'iconRight');
        ...
        var iconType = sLoading ? 'loading' : icon;
        var iconComponent = flaticon
            ? sLoading
                ? h(Icon, { attrs: { type: iconType } })
                : h('i', { class: `flaticon fi ${iconType} ${iconClass}` })
            : h(Icon, { attrs: { type: iconType } });
        ...
        var content = iconRight ? [...kids, iconNode] : [iconNode, ...kids];
