...
export default {
    ...
    data() {
        return {
            ...
            listLoading: false,
            actions: null,
            listModel: "help_desk.HelpDeskTicketCategoryModel",
        }
    },
    computed: {
        hasNoActions() {
    ...
    methods: {
        editItem() {
            eventBus.$emit('open_modal_category_create', { record: this.record })
        ...
        async getActions() {
            if(!this.actions) {
                try {
                    ...
                    const { data } = await this.$http.get(`help_desk/ticket_categories/${this.record.id}{/action_info/`)
                    if(data?.actions) {
