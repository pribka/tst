...
from risk_assessment import models
...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        parser.add_argument(
            ...
            action='store_true',
            help='Rebuild areas in risk assessment local points.'
        )

    def handle(self, *args, **options):
        if options['rebuild_location_points']:
            qs = models.RiskAssessmentModel.objects.filter(is_active=True, location_points__isnull=False).distinct()
            for each in qs:
