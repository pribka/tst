...
def user_context(user):
    """
    Менеджер контекста для временной установки текущего пользователя.
    Гарантирует автоматическую очистку.

    Пример использования:
    with user_context(some_user):
        MyModel.objects.create(...)
    """

    # 1. Установка пользователя (логика, выполняемая при входе в 'with')
    original_user = get_current_user()  # Сохраняем предыдущего пользователя, если он был
    _set_current_user(user)

    try:
        # 2. Yield — выполнение кода внутри блока 'with'
        yield

    finally:
        # 3. Очистка (логика, выполняемая при выходе из 'with', даже при ошибке)
        # Восстанавливаем оригинального пользователя (или None, если его не было)
        _set_current_user(original_user)
