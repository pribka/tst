...
class InvestProjectModelPermission(BasePermission):
...
class UpdateInvestProjectStatusPermission(BasePermission):
    def has_object_permission(self, request, view, obj):
        return obj.get_update_status_permission(request)

class DeleteInvestProjectPermission(BasePermission):
    def has_object_permission(self, request, view, obj):
