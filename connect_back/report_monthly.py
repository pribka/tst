...
def bootstrap_django() -> None:
    global ContentType, Count, Q, TruncMonth, timezone, TaskModel, KlassificationCategoryModel, KlassificationModel
    ...
    from django.db.models.functions import TruncMonth as DjangoTruncMonth
...
def collect_report_data() -> Dict[str, object]:
    bootstrap_django()
    print("Loading monthly classification aggregates...", flush=True)

    task_ct = ContentType.objects.get_for_model(TaskModel)
    base_qs = (
        KlassificationModel.objects
        .filter(
            is_active=True,
        ...
        .annotate(task_month=TruncMonth("related_object__taskmodel__created_at"))
    )
    ...
    monthly_rows = list(
        base_qs.filter(category__isnull=False, task_month__isnull=False)
        .values(
            PROJECT_ID_FIELD,
            DIMENSION_ID_FIELD,
            CATEGORY_ID_FIELD,
    ...
    month_labels_by_dimension: Dict[Tuple[str, Optional[str]], set] = defaultdict(set)
    monthly_counts_by_bucket: Dict[Tuple[str, Optional[str], Optional[str], str], int] = {}
    for row in monthly_rows:
        raw_project_id = row[PROJECT_ID_FIELD]
        project_key = str(raw_project_id) if raw_project_id is not None else "__no_project__"
        raw_dimension_id = row[DIMENSION_ID_FIELD]
        raw_category_id = row[CATEGORY_ID_FIELD]
        month_value = row[TASK_MONTH_FIELD]
        if month_value is None:
            continue
        month_label = month_value.strftime("%Y-%m")
        month_labels_by_dimension[(project_key, raw_dimension_id)].add(month_label)
        monthly_counts_by_bucket[(project_key, raw_dimension_id, raw_category_id, month_label)] = int(row["tasks_count"])
    ...
    for row in dimension_rows:
        ...
        dimension_entry = {
            ...
            "month_labels": sorted(month_labels_by_dimension.get((project_key, raw_dimension_id), set())),
            "category_counts": {
    ...
    for project_entry in projects_map.values():
        project_key = str(project_entry["project_id"]) if project_entry["project_id"] is not None else "__no_project__"
        dimensions: List[DimensionRow] = []
        for dimension_id, dimension_entry in project_entry["dimensions"].items():
            ...
            categories = [
                CategoryRow(
                    ...
                    tasks=tasks_by_bucket.get((project_key, dimension_id, item["category_id"]), []),
                    monthly_counts={
                        month_label: monthly_counts_by_bucket.get(
                            (project_key, dimension_id, item["category_id"], month_label),
                            0,
                        )
                        for month_label in dimension_entry["month_labels"]
                    },
                )
            ...
            dimensions.append(
                DimensionRow(
                    ...
                    month_labels=dimension_entry["month_labels"],
...
def build_monthly_chart_html(
    ...
    positive_categories = [
        item for item in categories if any(item.monthly_counts.get(month_label, 0) > 0 for month_label in month_labels)
    ]
    if not month_labels or not positive_categories:
        return "<div class='empty-chart'>Нет помесячных данных по категориям для построения диаграммы.</div>"

    color_map = build_color_map(positive_categories)
    fig = go.Figure()
    for item in positive_categories:
        month_values = [item.monthly_counts.get(month_label, 0) for month_label in month_labels]
        fig.add_trace(
            go.Bar(
                name=item.category_name,
                x=month_labels,
                y=month_values,
                customdata=month_values,
                marker={"color": color_map[item.category_id]},
                hovertemplate=(
                    f"Категория: {escape(item.category_name)}<br>"
                    "Месяц: %{x}<br>"
                    "Задач: %{customdata}<extra></extra>"
                ),
            )
        )
    ...
    fig.update_layout(
        ...
        height=500,
        margin={"t": 16, "r": 20, "b": 100, "l": 70},
        barmode="stack",
        xaxis_title="Месяц создания задачи",
        yaxis_title="Количество задач",
        showlegend=False,
        hovermode="closest",
    )
    fig.update_xaxes(type="category", tickangle=-35, automargin=True)
    ...
    if not projects:
        ...
        for project in projects:
            ...
            for dimension in project.dimensions:
                chart_html = build_monthly_chart_html(
                    project_name=project.project_name,
                    dimension_name=dimension.dimension_name,
                    month_labels=dimension.month_labels,
                ...
                note_parts = [
                    f"Классифицировано задач: {dimension.distinct_tasks}",
                    f"Месяцев на графике: {len(dimension.month_labels)}",
                ]
...
<body>
    <main class="page">
        <section class="hero">
            <h1>{escape(title)}</h1>
            <p>Отчет по динамике категорий задач по месяцам. Для каждого проекта и измерения строится нормализованная stacked bar диаграмма, где каждый месяц приводится к 100%, чтобы было видно именно изменение пропорций категорий.</p>
            <div class="note">
                Отчет сформирован: {escape(generated_at)}.<br>
                Фильтр проектов: названия содержат "Коннект" или "АУЦА". Месяц берется из <code>TaskModel.created_at</code> с усечением до начала месяца.
            </div>
            {summary_html}
        </section>
        {projects_html}
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Построить HTML-отчет по динамике классификаций задач по месяцам."
    )
    parser.add_argument(
        "--output",
        default="klassification_monthly_report.html",
